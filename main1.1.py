...
import json
def jsontogpx():
    with open('toGPX.json', 'r', -1, 'utf-8') as f:
        data = json.load(f)
        lat = data['record']['lat']
        lon = data['record']['lon']
        lenoflat = len(lat)
        list1 = []
        for i in range(lenoflat):
            point = {}
            for j in range(3):

                if j == 0:
                    point['name'] = 'point'+str(i+1)
                if j == 1:
                    point['lat'] = lat[i]
                if j == 2:
                    point['lon'] = lon[i]
...
#开始写xml文档
    fp = open('jsontoGPXresult.gpx', 'w')
    doc.writexml(fp, indent='\t', addindent='\t', newl='\n', encoding="utf-8")
    print('Successfully convert to GPX file')
